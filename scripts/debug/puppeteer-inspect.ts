...
async function main() {
  ...
  page.on("request", (req: HTTPRequest) => {
    ...
    // If user explicitly allowed this resource type, let it through
    ...
    if (rType === "stylesheet" || rType === "font") {
      blockedCount += 1;
    ...
    if (blockSet.has(rType)) {
      blockedCount += 1;
      rec.failure = "blocked-by-inspector";
      req.abort();
      return;
    }
