...
function extractTableNames(): string[] {
  ...
  for (let i = endIndex; i < content.length; i++) {
    ...
    if (content[i] === '}') {
      braceCount--;
      if (braceCount === -1) { // back to the Tables level
        endIndex = i;
        break;
      }
    }
  ...
  for (const line of lines) {
    ...
    if (trimmed && !trimmed.startsWith('//') && trimmed.includes(': {')) {
      ...
      if (tableName && tableName !== 'Row' && tableName !== 'Insert' && tableName !== 'Update') {
...
function parseTableRow(tableName: string, content: string): Record<string, string> {
  // Find the table section
  const tableRegex = new RegExp(`${tableName}:\\s*{([\\s\\S]*?)}\\s*},?\\s*(?=\\w+:|$)`, 'i');
...
function generateSchemasFromDatabase() {
  ...
  tableNames.forEach(tableName => {
    ...
    schemas.push(`export const ${schemaName} = ${generateZodSchema(tableName)};`);
...
console.log('Schemas and types generated dynamically from database.schema.ts! Written to lib/schemas.ts');