...
async function scrapeZeroHeightProject(url: string, password?: string) {
  try {
    ...
    if (password) {
      ...
      if (passwordInput) {
        ...
        const errorText = await page.$eval('body', (body) => {
          ...
          if (text.toLowerCase().includes('incorrect') && text.toLowerCase().includes('password')) {
          ...
          if (text.toLowerCase().includes('access denied') || text.toLowerCase().includes('unauthorized')) {
        ...
        const navLinks = await page.$$eval('nav a, .navigation a, .menu a', links => links.length);
    ...
    const hasContent = await page.$('.content, .zh-content, main, [data-testid*="content"], .page-content');
    console.log(`Content container found: ${!!hasContent}`);
    
    const bodyTextLength = await page.$eval('body', body => (body.textContent || '').length);
    ...
    const allRawLinks = await page.$$eval('a[href]', links => links.map(link => link.href));
    ...
    const pagesToInsert: Array<{url: string, title: string, content: string, images: Array<{src: string, alt: string, index: number}>}> = [];
    ...
    for (const link of allLinks) {
      ...
      try {
        ...
        const allImages = [...images, ...bgImages].filter((img): img is { src: string; alt: string; index: number } => Boolean(img));
        ...
        newLinks.forEach(newLink => {
          if (!allLinks.has(newLink)) {
            ...
            console.log(`Discovered new link: ${newLink} (total links now: ${allLinks.size})`);
          }
        });
    ...
    if (pagesToInsert.length > 0) {
      const { data: insertedPages, error: bulkError } = await client!.from("pages").insert(
        pagesToInsert.map(({ url, title, content }) => ({ url, title, content }))
      ).select();

      if (bulkError) {
        console.error("Error bulk inserting pages:", bulkError);
        return createErrorResponse(`Error saving pages to database: ${bulkError.message}`);
      }
      ...
      if (imagesToInsert.length > 0) {
        const { error: bulkImageError } = await client!.from("images").insert(imagesToInsert);
        if (bulkImageError) {
    ...
    return createErrorResponse("Error scraping project: " + (error instanceof Error ? error.message : String(error)));
  }
}
...
export const scrapeZeroheightProjectTool = {
  ...
  handler: async () => {
    ...
    if (!url) {
      return createErrorResponse("Error: ZEROHEIGHT_PROJECT_URL environment variable not set");
    }
...
export const queryZeroheightDataTool = {
  ...
  handler: async ({ search, url, includeImages, limit }: { search?: string; url?: string; includeImages?: boolean; limit?: number }) => {
    const client = getSupabaseClient();
    if (!client) {
